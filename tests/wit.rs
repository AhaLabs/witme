...
fn help_case() -> Result<()> {
  ...
  println!("{:#?}",command.current_dir("./examples/counter").arg("near").arg("wit"));
   Ok(())
}
...
fn simple_case() -> Result<()> {
  ...
  command.current_dir("./examples/counter").arg("near").arg("wit").arg("-o").arg(temp.join("index.wit")).unwrap();
